OPENQASM 2.0;
include "qelib1.inc";

qreg node[24];
creg c[20];
rz(0.5*pi) node[0];
sx node[0];
rz(1.0*pi) node[0];
cx node[0],node[1];
rz(3.75*pi) node[1];
cx node[0],node[1];
rz(0.75*pi) node[1];
sx node[1];
rz(0.9999981049135106*pi) node[1];
swap node[4],node[1];
cx node[0],node[1];
rz(3.875*pi) node[1];
cx node[0],node[1];
rz(0.125*pi) node[1];
cx node[4],node[1];
rz(3.75*pi) node[1];
cx node[4],node[1];
rz(0.75*pi) node[1];
sx node[1];
rz(0.9999961950541935*pi) node[1];
swap node[2],node[1];
cx node[0],node[1];
rz(3.9375*pi) node[1];
cx node[0],node[1];
rz(0.0625*pi) node[1];
cx node[4],node[1];
rz(3.875*pi) node[1];
cx node[4],node[1];
rz(0.125*pi) node[1];
swap node[7],node[4];
cx node[2],node[1];
rz(3.75*pi) node[1];
cx node[2],node[1];
rz(0.75*pi) node[1];
sx node[1];
rz(0.9999923753355593*pi) node[1];
swap node[0],node[1];
cx node[1],node[4];
rz(3.96875*pi) node[4];
cx node[1],node[4];
swap node[1],node[2];
rz(0.03125*pi) node[4];
cx node[2],node[3];
cx node[7],node[4];
rz(3.984375*pi) node[3];
rz(3.9375*pi) node[4];
cx node[2],node[3];
cx node[7],node[4];
rz(0.015625*pi) node[3];
rz(0.0625*pi) node[4];
cx node[1],node[4];
swap node[2],node[3];
cx node[3],node[5];
rz(3.875*pi) node[4];
cx node[1],node[4];
rz(3.9921875*pi) node[5];
swap node[2],node[1];
cx node[3],node[5];
rz(0.125*pi) node[4];
swap node[4],node[1];
rz(0.0078125*pi) node[5];
cx node[0],node[1];
swap node[3],node[5];
cx node[7],node[4];
rz(3.75*pi) node[1];
rz(3.96875*pi) node[4];
cx node[5],node[8];
cx node[0],node[1];
cx node[7],node[4];
rz(3.99609375*pi) node[8];
rz(0.75*pi) node[1];
rz(0.03125*pi) node[4];
cx node[5],node[8];
sx node[1];
swap node[4],node[7];
rz(0.00390625*pi) node[8];
rz(0.9999847358982908*pi) node[1];
swap node[8],node[5];
swap node[4],node[1];
cx node[8],node[11];
swap node[1],node[2];
swap node[7],node[4];
rz(3.998046875*pi) node[11];
cx node[1],node[4];
cx node[2],node[3];
cx node[8],node[11];
rz(3.984375*pi) node[3];
rz(3.9375*pi) node[4];
cx node[8],node[9];
rz(0.001953125*pi) node[11];
cx node[1],node[4];
cx node[2],node[3];
rz(3.9990234375*pi) node[9];
rz(0.015625*pi) node[3];
rz(0.0625*pi) node[4];
cx node[8],node[9];
swap node[2],node[3];
swap node[11],node[8];
rz(0.0009765625*pi) node[9];
cx node[1],node[2];
cx node[3],node[5];
cx node[11],node[14];
rz(3.96875*pi) node[2];
rz(3.9921875*pi) node[5];
rz(3.99951171875*pi) node[14];
cx node[1],node[2];
cx node[3],node[5];
cx node[11],node[14];
rz(0.03125*pi) node[2];
rz(0.0078125*pi) node[5];
rz(0.00048828125*pi) node[14];
swap node[1],node[2];
swap node[3],node[5];
swap node[14],node[11];
swap node[0],node[1];
cx node[2],node[3];
cx node[5],node[8];
cx node[14],node[13];
cx node[1],node[4];
rz(3.984375*pi) node[3];
rz(3.99609375*pi) node[8];
rz(3.999755859375*pi) node[13];
cx node[2],node[3];
rz(3.875*pi) node[4];
cx node[5],node[8];
cx node[14],node[13];
cx node[1],node[4];
rz(0.015625*pi) node[3];
rz(0.00390625*pi) node[8];
rz(0.000244140625*pi) node[13];
cx node[14],node[16];
cx node[1],node[0];
swap node[2],node[3];
rz(0.125*pi) node[4];
swap node[5],node[8];
rz(3.9998779296875*pi) node[16];
rz(3.9375*pi) node[0];
cx node[3],node[5];
cx node[7],node[4];
cx node[8],node[9];
cx node[14],node[16];
cx node[1],node[0];
rz(3.75*pi) node[4];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[9];
swap node[13],node[14];
rz(0.0001220703125*pi) node[16];
rz(0.0625*pi) node[0];
cx node[1],node[2];
cx node[3],node[5];
cx node[7],node[4];
cx node[8],node[9];
cx node[13],node[12];
rz(3.96875*pi) node[2];
rz(0.75*pi) node[4];
rz(0.0078125*pi) node[5];
cx node[8],node[11];
rz(0.001953125*pi) node[9];
rz(3.99993896484375*pi) node[12];
cx node[1],node[2];
swap node[3],node[5];
sx node[4];
rz(3.9990234375*pi) node[11];
cx node[13],node[12];
rz(0.03125*pi) node[2];
rz(0.9999694888547427*pi) node[4];
cx node[8],node[11];
rz(6.103515625e-05*pi) node[12];
swap node[1],node[2];
swap node[7],node[4];
rz(0.0009765625*pi) node[11];
swap node[13],node[12];
swap node[0],node[1];
cx node[2],node[3];
swap node[8],node[11];
cx node[12],node[10];
cx node[4],node[1];
rz(3.984375*pi) node[3];
swap node[5],node[8];
rz(3.999969482421875*pi) node[10];
cx node[11],node[14];
rz(3.875*pi) node[1];
cx node[2],node[3];
cx node[8],node[9];
cx node[12],node[10];
rz(3.99951171875*pi) node[14];
cx node[4],node[1];
rz(0.015625*pi) node[3];
rz(3.99609375*pi) node[9];
rz(3.0517578125e-05*pi) node[10];
cx node[11],node[14];
cx node[12],node[15];
rz(0.125*pi) node[1];
swap node[2],node[3];
cx node[8],node[9];
rz(0.00048828125*pi) node[14];
rz(3.9999847412109375*pi) node[15];
swap node[4],node[1];
cx node[8],node[5];
rz(0.00390625*pi) node[9];
swap node[11],node[14];
cx node[12],node[15];
cx node[1],node[0];
cx node[7],node[4];
rz(3.998046875*pi) node[5];
cx node[14],node[16];
rz(1.52587890625e-05*pi) node[15];
rz(3.9375*pi) node[0];
rz(3.75*pi) node[4];
cx node[8],node[5];
swap node[12],node[15];
rz(3.999755859375*pi) node[16];
cx node[1],node[0];
cx node[7],node[4];
rz(0.001953125*pi) node[5];
cx node[8],node[11];
cx node[14],node[16];
cx node[15],node[18];
rz(0.0625*pi) node[0];
cx node[1],node[2];
swap node[3],node[5];
rz(0.75*pi) node[4];
rz(3.9990234375*pi) node[11];
cx node[14],node[13];
rz(0.000244140625*pi) node[16];
rz(3.9999923706054688*pi) node[18];
rz(3.96875*pi) node[2];
sx node[4];
cx node[8],node[11];
rz(3.9998779296875*pi) node[13];
cx node[15],node[18];
cx node[1],node[2];
rz(0.9999389629366576*pi) node[4];
rz(0.0009765625*pi) node[11];
cx node[14],node[13];
rz(7.62939453125e-06*pi) node[18];
rz(0.03125*pi) node[2];
swap node[7],node[4];
swap node[8],node[11];
rz(0.0001220703125*pi) node[13];
swap node[18],node[15];
swap node[1],node[2];
swap node[9],node[8];
swap node[14],node[13];
cx node[18],node[21];
swap node[0],node[1];
cx node[5],node[8];
swap node[11],node[14];
swap node[13],node[12];
rz(3.9999961853027344*pi) node[21];
cx node[4],node[1];
rz(3.9921875*pi) node[8];
cx node[12],node[10];
cx node[14],node[16];
cx node[18],node[21];
rz(3.875*pi) node[1];
cx node[5],node[8];
rz(3.99993896484375*pi) node[10];
rz(3.99951171875*pi) node[16];
rz(3.814697265625e-06*pi) node[21];
cx node[4],node[1];
cx node[5],node[3];
rz(0.0078125*pi) node[8];
cx node[12],node[10];
cx node[14],node[16];
swap node[21],node[18];
rz(0.125*pi) node[1];
rz(3.99609375*pi) node[3];
rz(6.103515625e-05*pi) node[10];
cx node[14],node[11];
cx node[12],node[13];
rz(0.00048828125*pi) node[16];
swap node[23],node[21];
swap node[4],node[1];
cx node[5],node[3];
rz(3.999755859375*pi) node[11];
rz(3.999969482421875*pi) node[13];
cx node[1],node[0];
rz(0.00390625*pi) node[3];
cx node[7],node[4];
swap node[8],node[5];
cx node[14],node[11];
cx node[12],node[13];
rz(3.9375*pi) node[0];
swap node[5],node[3];
rz(3.75*pi) node[4];
cx node[8],node[9];
rz(0.000244140625*pi) node[11];
cx node[12],node[15];
rz(3.0517578125e-05*pi) node[13];
cx node[1],node[0];
cx node[2],node[3];
cx node[7],node[4];
rz(3.998046875*pi) node[9];
swap node[14],node[13];
rz(3.9999847412109375*pi) node[15];
rz(0.0625*pi) node[0];
rz(3.984375*pi) node[3];
rz(0.75*pi) node[4];
cx node[8],node[9];
cx node[12],node[15];
cx node[2],node[3];
sx node[4];
swap node[8],node[11];
rz(0.001953125*pi) node[9];
rz(1.52587890625e-05*pi) node[15];
rz(0.015625*pi) node[3];
rz(0.9998779429314762*pi) node[4];
swap node[12],node[15];
swap node[3],node[2];
swap node[7],node[4];
swap node[10],node[12];
cx node[15],node[18];
cx node[1],node[2];
cx node[3],node[5];
cx node[13],node[12];
rz(3.9999923706054688*pi) node[18];
rz(3.96875*pi) node[2];
rz(3.9921875*pi) node[5];
rz(3.9998779296875*pi) node[12];
cx node[15],node[18];
cx node[1],node[2];
cx node[3],node[5];
cx node[13],node[12];
rz(7.62939453125e-06*pi) node[18];
rz(0.03125*pi) node[2];
rz(0.0078125*pi) node[5];
rz(0.0001220703125*pi) node[12];
cx node[13],node[14];
swap node[15],node[18];
swap node[1],node[2];
swap node[5],node[3];
rz(3.99993896484375*pi) node[14];
cx node[18],node[17];
swap node[0],node[1];
cx node[2],node[3];
cx node[13],node[14];
rz(3.9999961853027344*pi) node[17];
cx node[4],node[1];
rz(3.984375*pi) node[3];
swap node[13],node[12];
rz(6.103515625e-05*pi) node[14];
cx node[18],node[17];
rz(3.875*pi) node[1];
cx node[2],node[3];
cx node[12],node[10];
swap node[16],node[14];
rz(3.814697265625e-06*pi) node[17];
cx node[4],node[1];
rz(0.015625*pi) node[3];
rz(3.999969482421875*pi) node[10];
cx node[11],node[14];
swap node[17],node[18];
rz(0.125*pi) node[1];
swap node[2],node[3];
cx node[12],node[10];
rz(3.9990234375*pi) node[14];
swap node[4],node[1];
rz(3.0517578125e-05*pi) node[10];
cx node[11],node[14];
cx node[12],node[15];
cx node[1],node[0];
cx node[7],node[4];
cx node[11],node[8];
rz(0.0009765625*pi) node[14];
rz(3.9999847412109375*pi) node[15];
rz(3.9375*pi) node[0];
rz(3.75*pi) node[4];
rz(3.99951171875*pi) node[8];
cx node[12],node[15];
cx node[1],node[0];
cx node[7],node[4];
cx node[11],node[8];
rz(1.52587890625e-05*pi) node[15];
rz(0.0625*pi) node[0];
cx node[1],node[2];
rz(0.75*pi) node[4];
rz(0.00048828125*pi) node[8];
swap node[14],node[11];
swap node[12],node[15];
rz(3.96875*pi) node[2];
sx node[4];
swap node[9],node[8];
swap node[10],node[12];
cx node[14],node[13];
cx node[15],node[18];
cx node[1],node[2];
rz(0.9997558710901248*pi) node[4];
cx node[5],node[8];
rz(3.999755859375*pi) node[13];
rz(3.9999923706054688*pi) node[18];
rz(0.03125*pi) node[2];
swap node[7],node[4];
rz(3.99609375*pi) node[8];
cx node[14],node[13];
cx node[15],node[18];
swap node[1],node[2];
cx node[5],node[8];
rz(0.000244140625*pi) node[13];
cx node[14],node[16];
rz(7.62939453125e-06*pi) node[18];
swap node[0],node[1];
rz(0.00390625*pi) node[8];
swap node[15],node[18];
rz(3.9998779296875*pi) node[16];
cx node[4],node[1];
swap node[8],node[5];
cx node[14],node[16];
cx node[18],node[21];
rz(3.875*pi) node[1];
cx node[3],node[5];
cx node[8],node[11];
swap node[13],node[14];
rz(0.0001220703125*pi) node[16];
rz(3.9999961853027344*pi) node[21];
cx node[4],node[1];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[11];
cx node[13],node[12];
cx node[18],node[21];
rz(0.125*pi) node[1];
cx node[3],node[5];
cx node[8],node[11];
rz(3.99993896484375*pi) node[12];
rz(3.814697265625e-06*pi) node[21];
swap node[4],node[1];
rz(0.0078125*pi) node[5];
cx node[8],node[9];
rz(0.001953125*pi) node[11];
cx node[13],node[12];
swap node[21],node[18];
cx node[1],node[0];
swap node[3],node[5];
cx node[7],node[4];
rz(3.9990234375*pi) node[9];
rz(6.103515625e-05*pi) node[12];
swap node[21],node[23];
rz(3.9375*pi) node[0];
cx node[2],node[3];
rz(3.75*pi) node[4];
cx node[8],node[9];
swap node[13],node[12];
cx node[1],node[0];
rz(3.984375*pi) node[3];
cx node[7],node[4];
swap node[8],node[11];
rz(0.0009765625*pi) node[9];
cx node[12],node[10];
rz(0.0625*pi) node[0];
cx node[2],node[3];
rz(0.75*pi) node[4];
cx node[5],node[8];
rz(3.999969482421875*pi) node[10];
cx node[11],node[14];
rz(0.015625*pi) node[3];
sx node[4];
rz(3.99609375*pi) node[8];
cx node[12],node[10];
rz(3.99951171875*pi) node[14];
swap node[2],node[3];
rz(0.9995117274074218*pi) node[4];
cx node[5],node[8];
rz(3.0517578125e-05*pi) node[10];
cx node[11],node[14];
cx node[12],node[15];
cx node[1],node[2];
swap node[7],node[4];
rz(0.00390625*pi) node[8];
rz(0.00048828125*pi) node[14];
rz(3.9999847412109375*pi) node[15];
rz(3.96875*pi) node[2];
swap node[8],node[5];
swap node[11],node[14];
cx node[12],node[15];
cx node[1],node[2];
cx node[3],node[5];
cx node[8],node[9];
cx node[14],node[16];
rz(1.52587890625e-05*pi) node[15];
rz(0.03125*pi) node[2];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[9];
swap node[12],node[15];
rz(3.999755859375*pi) node[16];
swap node[1],node[2];
cx node[3],node[5];
cx node[8],node[9];
swap node[10],node[12];
cx node[14],node[16];
cx node[15],node[18];
swap node[0],node[1];
rz(0.0078125*pi) node[5];
cx node[8],node[11];
rz(0.001953125*pi) node[9];
cx node[14],node[13];
rz(0.000244140625*pi) node[16];
rz(3.9999923706054688*pi) node[18];
cx node[4],node[1];
swap node[3],node[5];
rz(3.9990234375*pi) node[11];
rz(3.9998779296875*pi) node[13];
cx node[15],node[18];
rz(3.875*pi) node[1];
cx node[2],node[3];
cx node[8],node[11];
cx node[14],node[13];
rz(7.62939453125e-06*pi) node[18];
cx node[4],node[1];
rz(3.984375*pi) node[3];
rz(0.0009765625*pi) node[11];
rz(0.0001220703125*pi) node[13];
swap node[18],node[15];
rz(0.125*pi) node[1];
cx node[2],node[3];
swap node[8],node[11];
swap node[14],node[13];
swap node[4],node[1];
rz(0.015625*pi) node[3];
swap node[9],node[8];
cx node[13],node[12];
swap node[16],node[14];
cx node[1],node[0];
swap node[2],node[3];
cx node[7],node[4];
cx node[5],node[8];
cx node[11],node[14];
rz(3.99993896484375*pi) node[12];
rz(3.9375*pi) node[0];
rz(3.75*pi) node[4];
rz(3.99609375*pi) node[8];
cx node[13],node[12];
rz(3.99951171875*pi) node[14];
cx node[1],node[0];
cx node[7],node[4];
cx node[5],node[8];
cx node[11],node[14];
rz(6.103515625e-05*pi) node[12];
rz(0.0625*pi) node[0];
cx node[1],node[2];
rz(0.75*pi) node[4];
rz(0.00390625*pi) node[8];
swap node[13],node[12];
rz(0.00048828125*pi) node[14];
rz(3.96875*pi) node[2];
sx node[4];
swap node[8],node[5];
cx node[12],node[10];
swap node[14],node[11];
cx node[1],node[2];
cx node[3],node[5];
rz(0.9990234400420158*pi) node[4];
cx node[8],node[9];
rz(3.999969482421875*pi) node[10];
cx node[14],node[16];
rz(0.03125*pi) node[2];
swap node[7],node[4];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[9];
cx node[12],node[10];
rz(3.999755859375*pi) node[16];
swap node[1],node[2];
cx node[3],node[5];
cx node[8],node[9];
rz(3.0517578125e-05*pi) node[10];
cx node[12],node[15];
cx node[14],node[16];
swap node[0],node[1];
rz(0.0078125*pi) node[5];
cx node[8],node[11];
rz(0.001953125*pi) node[9];
cx node[14],node[13];
rz(3.9999847412109375*pi) node[15];
rz(0.000244140625*pi) node[16];
cx node[4],node[1];
swap node[3],node[5];
rz(3.9990234375*pi) node[11];
cx node[12],node[15];
rz(3.9998779296875*pi) node[13];
rz(3.875*pi) node[1];
cx node[2],node[3];
cx node[8],node[11];
swap node[10],node[12];
cx node[14],node[13];
rz(1.52587890625e-05*pi) node[15];
cx node[4],node[1];
rz(3.984375*pi) node[3];
rz(0.0009765625*pi) node[11];
rz(0.0001220703125*pi) node[13];
rz(0.125*pi) node[1];
cx node[2],node[3];
swap node[8],node[11];
swap node[14],node[13];
swap node[4],node[1];
rz(0.015625*pi) node[3];
swap node[9],node[8];
cx node[13],node[12];
swap node[16],node[14];
cx node[1],node[0];
swap node[2],node[3];
cx node[7],node[4];
cx node[5],node[8];
cx node[11],node[14];
rz(3.99993896484375*pi) node[12];
rz(3.9375*pi) node[0];
rz(3.75*pi) node[4];
rz(3.99609375*pi) node[8];
cx node[13],node[12];
rz(3.99951171875*pi) node[14];
cx node[1],node[0];
cx node[7],node[4];
cx node[5],node[8];
cx node[11],node[14];
rz(6.103515625e-05*pi) node[12];
rz(0.0625*pi) node[0];
cx node[1],node[2];
rz(0.75*pi) node[4];
rz(0.00390625*pi) node[8];
swap node[12],node[13];
rz(0.00048828125*pi) node[14];
rz(3.96875*pi) node[2];
sx node[4];
swap node[8],node[5];
swap node[14],node[11];
cx node[12],node[15];
cx node[1],node[2];
cx node[3],node[5];
rz(0.998046865311204*pi) node[4];
cx node[8],node[9];
cx node[14],node[16];
rz(3.999969482421875*pi) node[15];
rz(0.03125*pi) node[2];
swap node[7],node[4];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[9];
cx node[12],node[15];
rz(3.999755859375*pi) node[16];
swap node[1],node[2];
cx node[3],node[5];
cx node[8],node[9];
cx node[14],node[16];
rz(3.0517578125e-05*pi) node[15];
swap node[0],node[1];
rz(0.0078125*pi) node[5];
cx node[8],node[11];
rz(0.001953125*pi) node[9];
swap node[15],node[12];
cx node[14],node[13];
rz(0.000244140625*pi) node[16];
cx node[4],node[1];
swap node[3],node[5];
rz(3.9990234375*pi) node[11];
rz(3.9998779296875*pi) node[13];
rz(3.875*pi) node[1];
cx node[2],node[3];
cx node[8],node[11];
cx node[14],node[13];
cx node[4],node[1];
rz(3.984375*pi) node[3];
rz(0.0009765625*pi) node[11];
rz(0.0001220703125*pi) node[13];
rz(0.125*pi) node[1];
cx node[2],node[3];
swap node[8],node[11];
swap node[14],node[13];
swap node[4],node[1];
rz(0.015625*pi) node[3];
swap node[9],node[8];
cx node[13],node[12];
swap node[16],node[14];
cx node[1],node[0];
swap node[2],node[3];
cx node[7],node[4];
cx node[5],node[8];
cx node[11],node[14];
rz(3.99993896484375*pi) node[12];
rz(3.9375*pi) node[0];
rz(3.75*pi) node[4];
rz(3.99609375*pi) node[8];
cx node[13],node[12];
rz(3.99951171875*pi) node[14];
cx node[1],node[0];
cx node[7],node[4];
cx node[5],node[8];
cx node[11],node[14];
rz(6.103515625e-05*pi) node[12];
rz(0.0625*pi) node[0];
cx node[1],node[2];
rz(0.75*pi) node[4];
rz(0.00390625*pi) node[8];
swap node[12],node[13];
rz(0.00048828125*pi) node[14];
rz(3.96875*pi) node[2];
sx node[4];
swap node[8],node[5];
swap node[12],node[10];
swap node[14],node[11];
cx node[1],node[2];
cx node[3],node[5];
rz(0.9960937476805688*pi) node[4];
cx node[8],node[9];
cx node[14],node[16];
rz(0.03125*pi) node[2];
swap node[7],node[4];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[9];
rz(3.999755859375*pi) node[16];
swap node[1],node[2];
cx node[3],node[5];
cx node[8],node[9];
cx node[14],node[16];
swap node[0],node[1];
rz(0.0078125*pi) node[5];
cx node[8],node[11];
rz(0.001953125*pi) node[9];
cx node[14],node[13];
rz(0.000244140625*pi) node[16];
cx node[4],node[1];
swap node[3],node[5];
rz(3.9990234375*pi) node[11];
rz(3.9998779296875*pi) node[13];
rz(3.875*pi) node[1];
cx node[2],node[3];
cx node[8],node[11];
cx node[14],node[13];
cx node[4],node[1];
rz(3.984375*pi) node[3];
rz(0.0009765625*pi) node[11];
rz(0.0001220703125*pi) node[13];
swap node[16],node[14];
rz(0.125*pi) node[1];
cx node[2],node[3];
swap node[8],node[11];
swap node[4],node[1];
rz(0.015625*pi) node[3];
swap node[9],node[8];
cx node[11],node[14];
cx node[1],node[0];
swap node[2],node[3];
cx node[7],node[4];
cx node[5],node[8];
rz(3.99951171875*pi) node[14];
rz(3.9375*pi) node[0];
rz(3.75*pi) node[4];
rz(3.99609375*pi) node[8];
cx node[11],node[14];
cx node[1],node[0];
cx node[7],node[4];
cx node[5],node[8];
rz(0.00048828125*pi) node[14];
rz(0.0625*pi) node[0];
cx node[1],node[2];
rz(0.75*pi) node[4];
rz(0.00390625*pi) node[8];
swap node[14],node[11];
rz(3.96875*pi) node[2];
sx node[4];
swap node[8],node[5];
cx node[14],node[13];
cx node[1],node[2];
cx node[3],node[5];
rz(0.9921875124192987*pi) node[4];
cx node[8],node[9];
rz(3.999755859375*pi) node[13];
rz(0.03125*pi) node[2];
swap node[7],node[4];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[9];
cx node[14],node[13];
swap node[1],node[2];
cx node[3],node[5];
cx node[8],node[9];
rz(0.000244140625*pi) node[13];
swap node[0],node[1];
rz(0.0078125*pi) node[5];
cx node[8],node[11];
rz(0.001953125*pi) node[9];
swap node[13],node[14];
cx node[4],node[1];
swap node[3],node[5];
rz(3.9990234375*pi) node[11];
rz(3.875*pi) node[1];
cx node[2],node[3];
cx node[8],node[11];
cx node[4],node[1];
rz(3.984375*pi) node[3];
rz(0.0009765625*pi) node[11];
rz(0.125*pi) node[1];
cx node[2],node[3];
swap node[8],node[11];
swap node[4],node[1];
rz(0.015625*pi) node[3];
swap node[9],node[8];
cx node[11],node[14];
cx node[1],node[0];
swap node[2],node[3];
cx node[7],node[4];
cx node[5],node[8];
rz(3.99951171875*pi) node[14];
rz(3.9375*pi) node[0];
rz(3.75*pi) node[4];
rz(3.99609375*pi) node[8];
cx node[11],node[14];
cx node[1],node[0];
cx node[7],node[4];
cx node[5],node[8];
rz(0.00048828125*pi) node[14];
rz(0.0625*pi) node[0];
cx node[1],node[2];
rz(0.75*pi) node[4];
rz(0.00390625*pi) node[8];
swap node[14],node[11];
rz(3.96875*pi) node[2];
sx node[4];
swap node[8],node[5];
cx node[1],node[2];
cx node[3],node[5];
rz(0.9843750100657694*pi) node[4];
cx node[8],node[9];
rz(0.03125*pi) node[2];
swap node[7],node[4];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[9];
swap node[1],node[2];
cx node[3],node[5];
cx node[8],node[9];
swap node[0],node[1];
rz(0.0078125*pi) node[5];
cx node[8],node[11];
rz(0.001953125*pi) node[9];
cx node[4],node[1];
swap node[3],node[5];
rz(3.9990234375*pi) node[11];
rz(3.875*pi) node[1];
cx node[2],node[3];
cx node[8],node[11];
cx node[4],node[1];
rz(3.984375*pi) node[3];
rz(0.0009765625*pi) node[11];
rz(0.125*pi) node[1];
cx node[2],node[3];
swap node[8],node[11];
swap node[4],node[1];
rz(0.015625*pi) node[3];
swap node[9],node[8];
cx node[11],node[14];
cx node[1],node[0];
swap node[3],node[2];
cx node[7],node[4];
cx node[5],node[8];
cx node[14],node[11];
rz(3.9375*pi) node[0];
rz(3.75*pi) node[4];
rz(3.99609375*pi) node[8];
cx node[11],node[14];
cx node[1],node[0];
cx node[7],node[4];
cx node[5],node[8];
swap node[13],node[14];
rz(0.0625*pi) node[0];
cx node[1],node[2];
rz(0.75*pi) node[4];
rz(0.00390625*pi) node[8];
swap node[14],node[11];
rz(3.96875*pi) node[2];
sx node[4];
swap node[8],node[5];
swap node[16],node[14];
cx node[1],node[2];
cx node[3],node[5];
rz(0.9687500053587114*pi) node[4];
cx node[8],node[9];
swap node[0],node[1];
rz(0.03125*pi) node[2];
swap node[7],node[4];
rz(3.9921875*pi) node[5];
rz(3.998046875*pi) node[9];
cx node[4],node[1];
cx node[3],node[5];
cx node[8],node[9];
rz(3.875*pi) node[1];
rz(0.0078125*pi) node[5];
cx node[8],node[11];
rz(0.001953125*pi) node[9];
cx node[4],node[1];
swap node[5],node[3];
cx node[11],node[8];
rz(0.125*pi) node[1];
cx node[8],node[11];
swap node[1],node[4];
swap node[9],node[8];
swap node[14],node[11];
cx node[1],node[2];
cx node[7],node[4];
cx node[5],node[8];
rz(3.9375*pi) node[2];
rz(3.75*pi) node[4];
rz(3.99609375*pi) node[8];
cx node[1],node[2];
cx node[7],node[4];
cx node[5],node[8];
rz(0.0625*pi) node[2];
rz(0.75*pi) node[4];
rz(0.00390625*pi) node[8];
swap node[2],node[1];
sx node[4];
swap node[5],node[8];
swap node[3],node[2];
rz(0.9375*pi) node[4];
cx node[8],node[11];
swap node[1],node[4];
cx node[11],node[8];
cx node[7],node[4];
cx node[8],node[11];
rz(3.875*pi) node[4];
cx node[7],node[4];
rz(0.125*pi) node[4];
cx node[1],node[4];
rz(3.75*pi) node[4];
cx node[1],node[4];
swap node[0],node[1];
rz(0.75*pi) node[4];
cx node[1],node[2];
sx node[4];
rz(3.984375*pi) node[2];
rz(0.875*pi) node[4];
cx node[1],node[2];
swap node[7],node[4];
rz(0.015625*pi) node[2];
cx node[3],node[2];
rz(3.96875*pi) node[2];
cx node[3],node[2];
rz(0.03125*pi) node[2];
swap node[5],node[3];
swap node[1],node[2];
cx node[2],node[3];
rz(3.9921875*pi) node[3];
cx node[2],node[3];
swap node[1],node[2];
rz(0.0078125*pi) node[3];
swap node[1],node[4];
cx node[5],node[3];
cx node[1],node[2];
rz(3.984375*pi) node[3];
rz(3.9375*pi) node[2];
cx node[5],node[3];
cx node[1],node[2];
rz(0.015625*pi) node[3];
swap node[5],node[8];
rz(0.0625*pi) node[2];
swap node[8],node[11];
swap node[2],node[1];
swap node[11],node[14];
cx node[0],node[1];
cx node[2],node[3];
swap node[14],node[13];
rz(3.875*pi) node[1];
rz(3.96875*pi) node[3];
cx node[0],node[1];
cx node[2],node[3];
rz(0.125*pi) node[1];
rz(0.03125*pi) node[3];
swap node[1],node[2];
swap node[4],node[1];
swap node[7],node[4];
swap node[1],node[4];
swap node[10],node[7];
cx node[1],node[2];
cx node[4],node[7];
cx node[10],node[12];
rz(3.75*pi) node[2];
cx node[7],node[4];
cx node[12],node[10];
cx node[1],node[2];
cx node[4],node[7];
cx node[10],node[12];
swap node[0],node[1];
rz(0.75*pi) node[2];
sx node[2];
rz(0.75*pi) node[2];
swap node[3],node[2];
cx node[1],node[2];
rz(3.9375*pi) node[2];
cx node[1],node[2];
swap node[1],node[4];
rz(0.0625*pi) node[2];
swap node[0],node[1];
swap node[4],node[7];
cx node[1],node[2];
swap node[7],node[10];
rz(3.875*pi) node[2];
swap node[10],node[12];
cx node[1],node[2];
swap node[12],node[15];
rz(0.125*pi) node[2];
cx node[13],node[12];
cx node[15],node[18];
cx node[3],node[2];
cx node[12],node[13];
cx node[18],node[15];
rz(3.75*pi) node[2];
cx node[13],node[12];
cx node[15],node[18];
cx node[3],node[2];
swap node[17],node[18];
rz(0.75*pi) node[2];
swap node[18],node[15];
sx node[2];
swap node[15],node[12];
swap node[21],node[18];
rz(0.5*pi) node[2];
swap node[12],node[13];
swap node[18],node[15];
swap node[23],node[21];
swap node[15],node[12];
swap node[13],node[14];
swap node[21],node[18];
swap node[12],node[10];
swap node[14],node[11];
swap node[18],node[15];
swap node[10],node[7];
swap node[11],node[8];
swap node[15],node[12];
swap node[7],node[4];
swap node[8],node[5];
swap node[12],node[10];
swap node[1],node[4];
cx node[3],node[5];
swap node[10],node[7];
cx node[2],node[1];
cx node[5],node[3];
cx node[4],node[7];
cx node[1],node[2];
cx node[3],node[5];
cx node[7],node[4];
cx node[2],node[1];
cx node[4],node[7];
barrier node[2],node[3],node[4],node[23],node[12],node[18],node[0],node[8],node[9],node[16],node[13],node[14],node[11],node[10],node[21],node[15],node[17],node[7],node[5],node[1];
